...
function loadStatsPage() {
	...
	var table = document.getElementById("stats_table");//Retrieve our table element
		...
		{//Outer for loop iterates over each row
				home_score = table.rows[row_counter].cells[2].innerHTML;//Read in a cells current value
...
function loadPlayersPage() {
	...
	for(var i = 0; i < players.length; i++) {
		arefs += '<a class="dropdown-item" onClick="switchPlayers('+ i +')" href="#">'+ players[i].name + '</a>';
		document.getElementById("selectPlayerButton").innerHTML = arefs;
	}
};
